...
import {withTheme,Text, IconButton, Button} from 'react-native-paper';
...
class GroupMember extends React.Component{
  ...
  componentDidMount(){
    firebase.firestore().collection('users').doc(this.props.user).onSnapshot((user) => {
        ...
        >
            <Block row middle style={styles.container}>
                <Block flex row middle>
                    <ProfilePic size={40} addEnabled={false} proPicUrl={this.props.picture} />
                ...
                </Block>
                {
                    ...
                    ? (
                        <FollowButton userId={this.state.user.id} followers={this.state.user.followers} />
                    )
                    : null
                }
            </Block>
