    ...
    firebase.firestore().collection('users').doc(firebase.auth().currentUser.uid).get()
      .then((currentUser) => {
        ...
        Promise.all([
          ...
          .where('location.latitude', '<', currentUser.data().location.latitude + (2*(1/69)))
          .where('location.latitude', '>', currentUser.data().location.latitude - (2*(1/69)))
          .get()
          .then((users) => {
              users.forEach((user) => {
          ...
          firebase.firestore().collection('users')
          .where('location.longitude', '<', currentUser.data().location.longitude + (2*(1/69)))
          .where('location.longitude', '>', currentUser.data().location.longitude - (2*(1/69)))
          .get()
          .then((users) => {
              users.forEach((user) => {
  ...
  onCreate = () => {
    ...
    firebase.firestore().collection('users').doc(firebase.auth().currentUser.uid).get()
      .then((doc) => {
        ...
        .then((game) => {
          Promise.all([
            firebase.firestore().collection('games').doc(game.id).collection('messages').doc().set({
      ...
      <>
        <Block center middle style={[styles.registerContainer, {backgroundColor:colors.dBlue,borderColor:colors.orange}]}>
          ...
          <MenuBlock
            ...
            title='Sport'
            items={['Basketball', 'Soccer', 'Spikeball', 'Football', 'Volleyball', 'Frisbee']}
          ...
          <ButtonBlock text='Create Game' onPress={this.onCreate} disabled={this.state.sport == null || this.state.intensity == null || this.state.location == null || this.state.time == null} disabledStyles={{opacity: .3, backgroundColor:colors.orange}} />
        </Block>
      </>
    );
  }
