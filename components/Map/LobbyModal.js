    ...
    if(marker != undefined){
      return (
        <Block column style={[styles.modalContainer, { backgroundColor: colors.dBlue, borderTopWidth: 2, borderTopColor: colors.orange }]}>
          ...
          <Text style={{ color: colors.grey, textAlign: "center", marginBottom:2 }} theme={{fonts:{medium:this.props.theme.fonts.regular}}}>{`Owner: @${marker.owner.username}`}</Text>
          <Text style={{ color: colors.grey, textAlign: "center",marginBottom:2 }} theme={{fonts:{medium:this.props.theme.fonts.regular}}}>{`Time: ${marker.startTime.timeString}`}</Text>
          <Text style={{ color: colors.grey, textAlign: "center",marginBottom:2 }}>{`Location: ${marker.locationName}`}</Text>
          <Block column middle style={{ padding: 8 }}>
              ...
              <Block row style={{ flexWrap: "wrap" }}>
                {
                  this.props.marker.players.map((player, index) => {
                    return (
                      <Block key={index} style={{ flexBasis: "50%", paddingHorizontal: 4, paddingTop: 4 }}>
            ...
            <Button
              mode="contained"
              dark={true}
              disabled={!marker.gameState == 'created' || (getDistance(this.props.userLoc, this.props.marker.location) * 0.000621371) > 10 || this.props.user.calendar.includes(marker.id)}
              onPress={() => { this.props.addToTeam(marker.id) }}
              style={[styles.joinButton, !marker.gameState == 'created' || (getDistance(this.props.userLoc, this.props.marker.location) * 0.000621371) > 10 || this.props.user.calendar.includes(marker.id) ? styles.disabled : null]}
