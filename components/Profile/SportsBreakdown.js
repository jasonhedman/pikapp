...
class SportsBreakdown extends React.Component {
  ...
  componentDidMount() {
    let sportsSorted = Object.keys(this.props.user.sports).sort((a, b) => {
      return this.props.user.sports[a] - this.props.user.sports[b];
    });
        ...
        {this.state.loaded ? (
          <Block style={{ marginBottom: 12 }}>
            <Block style={styles.favoriteSport}>
              <Subheading
                style={{ color: "#83838A", textAlign: "center", margin: 0 }}
              >
                Favorite Sport
              </Subheading>
              <Headline style={styles.stat}>
                {this.state.sports[0][0].toUpperCase() +
                  this.state.sports[0].substring(1)}
              </Headline>
            </Block>
              ...
              {this.state.sports.map((item, key) => {
                return (
                  <Block style={styles.sport} key={key}>
                    <Subheading style={styles.sportName}>
                    ...
                    >
                      ...
                      <Headline style={styles.stat}>
                        {this.props.user.sports[item].gamesPlayed}
                      </Headline>
                    </Block>
                  </Block>
                );
